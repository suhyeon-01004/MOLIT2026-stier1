...
전체 누적 성과와 달리 아래 표는 무엇을 바꿨을 때 어떤 지표가 움직였는지를 보여준다.
통제 조건이 다른 결과를 더하거나 곱해 전체 개선율로 만들지 않았다.

| 변경 사항 | 비교 설계 | 개선된 방향 | 악화·한계 | 판단 |
| --- | --- | --- | --- | --- |
| MPC 기준 yaw 계산 반경 `±0.25 → ±2.5 m` | 동일 경로·설정, 40초 3회 대 3회 | 급곡선/중곡률 조향 명령 변화율 RMS **19.75%/34.92% 감소**, 첫 좌회전 **32.80% 감소**, 추정 jerk **20.75% 감소**, 첫 우회전 최저속도 **6.66% 증가** | 직선 조향 변화율 4.86%, 급곡선 raw-GPS CTE 19.30% 증가 | 이후 설계 근거로 채택. 현재 최종 yaw 설정 자체의 독립 A/B는 아님 |
| RDDF–MGeo 분기·합류 3개 지점 형상 보정 | 동일 제어기·로컬라이제이션, 1회 대 1회 완주 | 문제 지점 heading 오차 **68.84%/59.56%/85.95% 감소**, 최저속도 **24.41%/4.18%/80.03% 증가**, 두 지점 국소 CTE **54.83%/52.61% 감소** | 전 구간 CTE 0.65%, 다른 한 지점 국소 CTE 156.03%, 1593 m 추정 차체 여유 34.78% 악화 | 장점과 회귀를 함께 기록하고 채택 |
| 앞축 오차 비용 `q=0 → 0.5` 및 오버행 0.845 m 반영 | 기존 3회 대 후보 2회, 동일 경로 | 최저속도 2.07% 증가 | 앞축 근사 여유 **6.67%**, 차체 근사 여유 **79.49% 감소**, brake 2.01%, 조향 변화율 1.01% 증가 | 폐기, 기본값 `q=0` 유지 |
| GPS·IMU·Competition 속도 기반 상태추정 보완 | 독립 전후 주행 없음 | 최종 누적 CTE·heading 결과에 포함 | 경로와 제어기도 함께 변경되어 로컬라이제이션 단독 개선율 분리 불가 | 운영 반영, 독립 개선율 미주장 |

즉, 자소서에는 `전체 누적 개선`과 `변경별 통제 결과`를 분리해 쓴다. 예를 들어
raw-GPS CTE RMS 31.19% 감소는 초기 스택 대비 최종 스택의 누적 결과이고, 곡선 조향
변화율 19.75~34.92% 감소는 기준 yaw 범위를 바꾼 반복 비교 결과다. 경로 보정은
전 구간 CTE 개선이 아니라 특정 분기·합류 지점의 heading·속도·국소 CTE 개선으로
표현한다.
