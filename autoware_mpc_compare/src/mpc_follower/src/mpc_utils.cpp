...
Eigen::Matrix2d MPCUtils::frontPointWeight(double weight, double distance)
{
  if (!std::isfinite(weight) || weight<0. || weight>100. ||
      !std::isfinite(distance) || distance<0. || distance>8.)
    throw std::invalid_argument("Invalid front-point cost");
  const Eigen::Vector2d projection(1.,distance);
  return weight * projection * projection.transpose();
}
...
void MPCUtils::calcTrajectoryYawFromXY(MPCTrajectory &traj, int half_window)
{
  if (half_window < 1) throw std::invalid_argument("Yaw half-window must be positive");
  if (traj.yaw.size() == 0)
    return;

  for (unsigned int i = 1; i < traj.yaw.size() - 1; ++i)
  {
    const unsigned int window = std::min(static_cast<unsigned int>(half_window),
        std::min(i, static_cast<unsigned int>(traj.yaw.size()-1-i)));
    const double dx = traj.x[i + window] - traj.x[i - window];
    const double dy = traj.y[i + window] - traj.y[i - window];
