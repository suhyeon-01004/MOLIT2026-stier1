...
// PSD cost for lateral error of a forward vehicle point, relative to the
// same reference pose: weight * (e_y + distance * e_yaw)^2.
Eigen::Matrix2d frontPointWeight(double weight, double distance);
 ...
 */
void calcTrajectoryYawFromXY(MPCTrajectory &traj, int half_window = 1);
