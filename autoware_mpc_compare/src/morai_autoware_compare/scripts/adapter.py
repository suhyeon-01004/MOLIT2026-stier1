#!/usr/bin/python3
"""MORAI adapter for the locally adapted Autoware AI MPC comparison.
...
class Adapter:
    def __init__(self):
        ...
        self.pub_lane = rospy.Publisher('/comparison/waypoints', Lane, queue_size=1)
        self.pub_pose = rospy.Publisher('/comparison/pose', PoseStamped, queue_size=1)
        self.pub_vehicle = rospy.Publisher('/comparison/vehicle_status_estimated', VehicleStatus, queue_size=1)
        self.pub_twist = rospy.Publisher('/comparison/twist', TwistStamped, queue_size=1)
        self.pub_command = rospy.Publisher('/control/actuator_command', ActuatorCommand, queue_size=1)
        self.pub_sent = rospy.Publisher('/comparison/sent_control', ControlCommandStamped, queue_size=1)
        ...
        for name, topic, kind in [
            ('long', '/comparison/custom_status', ControllerStatus),
            ('mpc', '/comparison/ctrl_raw', ControlCommandStamped),
            ('time', '/autoware_mpc/debug/mpc_calc_time', Float32)]:
            self.subs.append(rospy.Subscriber(topic, kind, lambda msg, n=name: self.store(n, msg), queue_size=1))
        self.path_sub = message_filters.Subscriber('/local_path', Path, queue_size=5)
        self.odom_sub = message_filters.Subscriber('/localization/odometry', Odometry, queue_size=5)
    ...
    def inputs(self, path, odom):
        with self.lock:
            try:
                ...
                if self.predictor:
                    vehicle.angle = self.predictor.advance(time.monotonic())
                self.pub_pose.publish(PoseStamped(header=odom.header, pose=odom.pose.pose))
                self.pub_vehicle.publish(vehicle)
                self.pub_twist.publish(TwistStamped(header=odom.header, twist=twist))
...
if __name__ == '__main__':
    rospy.init_node('autoware_adapter')
    Adapter()
    rospy.spin()
