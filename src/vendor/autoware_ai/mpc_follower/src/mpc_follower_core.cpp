...
#include "mpc_follower/curvature_weight_blend.h"
#include "mpc_follower/prediction_length.h"
    ...
    : nh_(""), pnh_("~"), my_position_ok_(false), my_velocity_ok_(false), my_steering_ok_(false)
{
  ...
  pnh_.param("morai_state_time_alignment", align_state_time_, false);
  ...
  pnh_.param("morai_straight_lat_weight_scale", straight_lat_scale_, 1.0);
  pnh_.param("morai_curve_steer_weight_scale", curve_steer_scale_, 1.0);
  pnh_.param("morai_weight_curvature_low", weight_curvature_low_, 0.005);
  pnh_.param("morai_weight_curvature_high", weight_curvature_high_, 0.03);
  pnh_.param("morai_use_output_steering_filter", use_output_steering_filter_, true);
  pnh_.param("morai_min_prediction_length_m", minimum_prediction_length_m_, 0.0);
  pnh_.param("morai_max_prediction_horizon", maximum_prediction_steps_, 90);
  pnh_.param("morai_yaw_smoothing_num", yaw_smoothing_num_, 1);
  if (yaw_smoothing_num_<1 || yaw_smoothing_num_>50)
    throw std::invalid_argument("Invalid reference-yaw window");
  pnh_.param("morai_weight_first_steer_change", first_steer_change_weight_, 0.0);
  if (!std::isfinite(first_steer_change_weight_) || first_steer_change_weight_<0. || first_steer_change_weight_>100.)
    throw std::invalid_argument("Invalid first steering-change weight");
  if (maximum_prediction_steps_>150 || minimum_prediction_length_m_>50.)
    throw std::invalid_argument("Prediction length exceeds comparison compute budget");
  moraiPredictionSteps(mpc_param_.prediction_horizon, maximum_prediction_steps_,
      mpc_param_.prediction_sampling_time, 0., minimum_prediction_length_m_);
  moraiCurvatureWeightBlend(0., weight_curvature_low_, weight_curvature_high_);
  if (!std::isfinite(straight_lat_scale_) || straight_lat_scale_ <= 0. || straight_lat_scale_ > 1. ||
      !std::isfinite(curve_steer_scale_) || curve_steer_scale_ <= 0. || curve_steer_scale_ > 1.)
    throw std::invalid_argument("Invalid curvature weight scales");

  pnh_.param("steer_lim_deg", steer_lim_deg_, double(35.0));
  pnh_.param("vehicle_model_wheelbase", wheelbase_, double(2.9));
  double front_weight, front_overhang;
  pnh_.param("morai_weight_front_point_error", front_weight, 0.0);
  pnh_.param("morai_front_overhang_m", front_overhang, 0.0);
  if (!std::isfinite(front_overhang) || front_overhang<0. || front_overhang>3.)
    throw std::invalid_argument("Invalid front overhang");
  front_point_weight_=MPCUtils::frontPointWeight(front_weight, wheelbase_+front_overhang);
  ...
  if (vehicle_model_type_ == "kinematics")
  {
    ...
    double steer_gain;
    pnh_.param("vehicle_model_steer_gain", steer_gain, 1.0);
    state_steer_tau_=steer_tau; state_steer_gain_=steer_gain;
    vehicle_model_ptr_ = std::make_shared<KinematicsBicycleModel>(wheelbase_, amathutils::deg2rad(steer_lim_deg_), steer_tau, steer_gain);
  ...
  if (align_state_time_ && vehicle_model_type_!="kinematics")
    throw std::invalid_argument("Time alignment requires command-history kinematics model");
  ...
  if (align_state_time_) {
    const auto hints=ros::TransportHints().tcpNoDelay();
    aligned_pose_sub_.subscribe(nh_,in_selfpose,5,hints);
    aligned_vehicle_sub_.subscribe(nh_,in_vehicle_status,5,hints);
    aligned_twist_sub_.subscribe(nh_,"estimate_twist",5,hints);
    aligned_lane_sub_.subscribe(nh_,in_waypoints,5,hints);
    state_sync_.reset(new StateSync(aligned_pose_sub_,aligned_vehicle_sub_,aligned_twist_sub_,aligned_lane_sub_,10));
    state_sync_->registerCallback(boost::bind(&MPCFollower::callbackAlignedState,this,_1,_2,_3,_4));
  } else {
  ...
  if (first_steer_change_weight_>0. || align_state_time_)
    sub_sent_control_ = nh_.subscribe("sent_control", 1, &MPCFollower::callbackSentControl, this,
        ros::TransportHints().tcpNoDelay(align_state_time_));
...
void MPCFollower::callbackSentControl(const autoware_msgs::ControlCommandStamped &msg)
{
  const double age=(ros::Time::now()-msg.header.stamp).toSec();
  last_sent_steer_valid_=std::isfinite(msg.cmd.steering_angle) &&
      std::abs(msg.cmd.steering_angle)<=amathutils::deg2rad(steer_lim_deg_)+1e-6 && age>=0. && age<=.25;
  if (last_sent_steer_valid_) {
    last_sent_steer_=msg.cmd.steering_angle;
    last_sent_received_=ros::SteadyTime::now();
    if (align_state_time_) {
      const double stamp=msg.header.stamp.toSec();
      if (!sent_history_.empty() && stamp<=sent_history_.back().stamp) {
        last_sent_steer_valid_=false;
        return;
      }
      sent_history_.push_back({stamp,last_sent_steer_});
      while (sent_history_.size()>1 && sent_history_[1].stamp<stamp-1.) sent_history_.pop_front();
    }
  }
}

void MPCFollower::callbackAlignedState(const geometry_msgs::PoseStamped::ConstPtr& pose,
    const autoware_msgs::VehicleStatus::ConstPtr& vehicle,const geometry_msgs::TwistStamped::ConstPtr& twist,
    const autoware_msgs::Lane::ConstPtr& lane)
{
  const auto& q=pose->pose.orientation;
  const double norm=q.x*q.x+q.y*q.y+q.z*q.z+q.w*q.w;
  const double age=(ros::Time::now()-pose->header.stamp).toSec();
  if (pose->header.stamp.isZero() || age<0. || age>.15 ||
      pose->header.frame_id!="map" || lane->header.frame_id!="map" ||
      vehicle->header.frame_id!="base_link" || twist->header.frame_id!="base_link" ||
      !std::isfinite(norm) || std::abs(norm-1.)>.01 ||
      !std::isfinite(vehicle->angle) || std::abs(vehicle->angle)>1.1 ||
      !std::isfinite(vehicle->speed) || std::abs(vehicle->speed)>180. ||
      !std::isfinite(twist->twist.linear.x) || !std::isfinite(twist->twist.angular.z) ||
      std::abs(twist->twist.angular.z)>3. || std::abs(vehicle->speed/3.6-twist->twist.linear.x)>1e-5 ||
      !std::isfinite(pose->pose.position.x) || !std::isfinite(pose->pose.position.y) || lane->waypoints.size()<20) {
    my_position_ok_=false;
    ROS_WARN_THROTTLE(1.,"[MPC] rejected aligned input generation");
    return;
  }
  callbackRefPath(lane);
  callbackPose(pose);
  callbackVehicleStatus(*vehicle);
  estimate_twist_=*twist;
  vehicle_status_.twist.angular=twist->twist.angular;
  aligned_received_=ros::SteadyTime::now();
}

bool MPCFollower::calculateMPC(double &vel_cmd, double &acc_cmd, double &steer_cmd, double &steer_vel_cmd)
{
  const double calculation_time=ros::Time::now().toSec();
  const double state_age=calculation_time-vehicle_status_.header.stamp.toSec();
  geometry_msgs::Pose control_pose=vehicle_status_.pose;
  double steer=vehicle_status_.tire_angle_rad;
  if (align_state_time_) {
    try {
      if ((ros::SteadyTime::now()-aligned_received_).toSec()>.15)
        throw std::invalid_argument("Aligned input receipt stale");
      const auto predicted=state_time::predictPose({control_pose.position.x,control_pose.position.y,
          tf2::getYaw(control_pose.orientation)},vehicle_status_.twist.linear.x,vehicle_status_.twist.angular.z,state_age);
      control_pose.position.x=predicted.x; control_pose.position.y=predicted.y;
      tf2::Quaternion q; q.setRPY(0.,0.,predicted.yaw);
      control_pose.orientation.x=q.x();control_pose.orientation.y=q.y();
      control_pose.orientation.z=q.z();control_pose.orientation.w=q.w();
      steer=state_time::predictSteering(steer,vehicle_status_.header.stamp.toSec(),calculation_time,
          sent_history_,mpc_param_.delay_compensation_time,state_steer_tau_,state_steer_gain_);
    } catch (const std::invalid_argument& error) {
      ROS_WARN_THROTTLE(1.,"[MPC] time alignment rejected: %s",error.what());
      return false;
    }
  }
  const double feedback_age=last_sent_steer_valid_ ? (ros::SteadyTime::now()-last_sent_received_).toSec() : -1.;
  if ((first_steer_change_weight_>0. || align_state_time_) && (feedback_age<0. || feedback_age>.25)) {
    ROS_WARN_THROTTLE(1., "[MPC] sent-command feedback missing/stale; stop MPC");
    return false;
  }
  const double DT = mpc_param_.prediction_sampling_time;
  const int N = moraiPredictionSteps(mpc_param_.prediction_horizon, maximum_prediction_steps_,
      DT, vehicle_status_.twist.linear.x, minimum_prediction_length_m_);
  ...
  const double current_yaw = tf2::getYaw(control_pose.orientation);
  ...
  if (!MPCUtils::calcNearestPoseInterp(ref_traj_, control_pose, nearest_pose, nearest_index, dist_err, yaw_err, nearest_traj_time))
  {
  ...
  const double err_x = control_pose.position.x - nearest_pose.position.x;
  const double err_y = control_pose.position.y - nearest_pose.position.y;
  const double sp_yaw = tf2::getYaw(nearest_pose.orientation);
  const double err_lat = -sin(sp_yaw) * err_x + cos(sp_yaw) * err_y;

  /* get steering angle */
  // Pose and steering model state share calculation_time when alignment is enabled.
  ...
  for (unsigned int i = 0; i < input_buffer_.size(); ++i)
  {
    ...
    ud(0, 0) = align_state_time_ ? state_time::commandAt(sent_history_,
        calculation_time-mpc_param_.delay_compensation_time+i*ctrl_period_) : input_buffer_.at(i);
  ...
  for (int i = 0; i < N; ++i)
  {
    ...
    const double curve_blend = moraiCurvatureWeightBlend(ref_k, weight_curvature_low_, weight_curvature_high_);
    Q_adaptive(0, 0) *= straight_lat_scale_ + (1.0-straight_lat_scale_) * curve_blend;
    R_adaptive(0, 0) *= 1.0 + (curve_steer_scale_-1.0) * curve_blend;
    // Same reference vehicle pose, not a shifted path: e_front ~= e_y + d*e_yaw.
    // ponytail: small-angle point cost; full-width body clearance stays an audit,
    // not a collision constraint. d includes the configured front overhang.
    Q_adaptive += front_point_weight_;
  ...
  // Only input-reference tracking contributes a feed-forward linear term.
  // The jerk penalty below damps actual command differences, not U-Uref differences.
  const Eigen::MatrixXd reference_input_gradient = -Urefex.transpose() * Rex;
  ...
  Eigen::MatrixXd f = (Cex * (Aex * x0 + Wex)).transpose() * QCB + reference_input_gradient;
  // First transition spans the control period, not the longer prediction step.
  // Feedback is the adapter's final bounded command, NOT measured tire angle.
  const double first_change_weight=first_steer_change_weight_ *
      vehicle_status_.twist.linear.x * vehicle_status_.twist.linear.x * DT / ctrl_period_;
  H(0,0)+=first_change_weight;
  f(0,0)-=first_change_weight*last_sent_steer_;
  ...
  // Optional MORAI comparison constraint: enforce rate INSIDE the prediction.
  double rate_deg = 0.0;
  pnh_.param("morai_steer_rate_limit_deg", rate_deg, 0.0);
  if (rate_deg > 0.0) {
    const double rate = amathutils::deg2rad(rate_deg);
    for (int i = 0; i < N; ++i) {
      A(i, i) = 1.0;
      if (i == 0) {
        const double previous=first_steer_change_weight_>0. ? last_sent_steer_ : steer_cmd_prev_;
        lbA(i) = previous - rate * ctrl_period_;
        ubA(i) = previous + rate * ctrl_period_;
      } else {
        A(i, i - 1) = -1.0;
        lbA(i) = -rate * DT;
        ubA(i) = rate * DT;
  ...
  /* filtering */
  // With rate bounds referenced to the last sent command, an extra output LPF
  // silently reduces achievable slew and adds dynamics absent from the model.
  const double u_filtered = use_output_steering_filter_ ? lpf_steering_cmd_.filter(u_sat) : u_sat;
  ...
  /* publish debug values */
  {
    ...
    debug_values.data.push_back(tf2::getYaw(control_pose.orientation));          // [6] control-state yaw
    ...
    debug_values.data.push_back(N);                                   // [17] actual horizon steps
    debug_values.data.push_back((N-1)*DT);                            // [18] reference time span [s]
    debug_values.data.push_back((N-1)*DT*std::max(.5,std::abs(curr_v))); // [19] constant-speed span [m]
    debug_values.data.push_back(last_sent_steer_);                     // [20] last bounded command, not tire angle
    debug_values.data.push_back(first_change_weight);                 // [21] first-transition cost coefficient
    debug_values.data.push_back(feedback_age);                        // [22] feedback receipt age [s], -1 unavailable
    debug_values.data.push_back(state_age);                           // [23] ORIGINAL input age, never relabeled fresh
    debug_values.data.push_back(align_state_time_ ? state_age : 0.);    // [24] bounded state prediction interval
    debug_values.data.push_back(align_state_time_ ? 1. : 0.);           // [25] synchronized state / prediction enabled
    debug_values.data.push_back(control_pose.position.x);             // [26] controller initial-state x, path unchanged
    debug_values.data.push_back(control_pose.position.y);             // [27] controller initial-state y
...
void MPCFollower::callbackRefPath(const autoware_msgs::Lane::ConstPtr &msg)
{
  ...
  {
    MPCUtils::calcTrajectoryYawFromXY(traj, yaw_smoothing_num_);
    MPCUtils::convertEulerAngleToMonotonic(traj.yaw);
  }
