...
private:
  ...
  double straight_lat_scale_{1.0}, curve_steer_scale_{1.0};
  double weight_curvature_low_{0.005}, weight_curvature_high_{0.03};
  bool use_output_steering_filter_{true};
  double minimum_prediction_length_m_{0.0};
  int maximum_prediction_steps_{90};
  int yaw_smoothing_num_{1};
  Eigen::Matrix2d front_point_weight_{Eigen::Matrix2d::Zero()};
  double first_steer_change_weight_{0.0}, last_sent_steer_{0.0};
  bool last_sent_steer_valid_{false};
  ros::SteadyTime last_sent_received_;
  ros::Subscriber sub_sent_control_;
  void callbackSentControl(const autoware_msgs::ControlCommandStamped &msg);
  bool align_state_time_{false};
  double state_steer_tau_{0.05}, state_steer_gain_{1.0};
  std::deque<state_time::Command> sent_history_;
  ros::SteadyTime aligned_received_;
  message_filters::Subscriber<geometry_msgs::PoseStamped> aligned_pose_sub_;
  message_filters::Subscriber<autoware_msgs::VehicleStatus> aligned_vehicle_sub_;
  message_filters::Subscriber<geometry_msgs::TwistStamped> aligned_twist_sub_;
  message_filters::Subscriber<autoware_msgs::Lane> aligned_lane_sub_;
  using StateSync=message_filters::TimeSynchronizer<geometry_msgs::PoseStamped,
      autoware_msgs::VehicleStatus,geometry_msgs::TwistStamped,autoware_msgs::Lane>;
  std::unique_ptr<StateSync> state_sync_;
  void callbackAlignedState(const geometry_msgs::PoseStamped::ConstPtr&,
      const autoware_msgs::VehicleStatus::ConstPtr&,const geometry_msgs::TwistStamped::ConstPtr&,
      const autoware_msgs::Lane::ConstPtr&);
